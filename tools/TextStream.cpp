...
extern tTJSBinaryStream * TVPCreateStream(const ttstr & name, tjs_uint32 flags);
extern ttstr TVPStringFromBMPUnicode(const tjs_uint16 *src, tjs_int maxlen,bool fix16);
...
//---------------------------------------------------------------------------
iTJSTextReadStream * TVPCreateTextStreamForReadWithStream(const ttstr & name,
	const ttstr & modestr, tTJSBinaryStream* s)
{
	return new tTVPTextReadStream(name, modestr,DefaultReadEncoding,s);
}
//---------------------------------------------------------------------------
