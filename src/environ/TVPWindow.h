...
protected:
	...
	virtual LRESULT Proc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam ){}

	HRESULT CreateWnd( const std::wstring& classname, const std::wstring& title, int width, int height, HWND hParent=NULL )
	{
		// TVPAddLog(TJS_W("create windows!!"));
		// init_sdl(&sdl_win_handle_, width,height);
	}
...
public:
	tTVPWindow()
	: window_handle_(NULL),created_(false), left_double_click_(false), border_style_(0), modal_result_(0),
	...
	WHAND GetHandle() { return sdl_win_handle_; }
	WHAND GetHandle() const { return sdl_win_handle_; }
