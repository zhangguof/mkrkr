...
//---------------------------------------------------------------------------
//change os native name to TVP path name.
void TVPPreNormalizeStorageName(ttstr &name)
{
	...
	if(namelen >= 2)
	{
		...
		//relative path, relative with cur_path
	}
...
ttstr TVPGetTemporaryName()
{
...

	{
		...
		if(!TVPTempPathInit)
		{
			...
			TVPProcessID = (tjs_int) boost::this_process::get_id();
...
tTJSBinaryStream * TVPOpenStream(const ttstr & _name, tjs_uint32 flags)
{
	...
	TVPAddLog(TJS_W("get local name:"));
	TVPAddLog(name);
