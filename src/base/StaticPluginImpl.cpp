...
void TVPAddImportFunction(const tjs_char *name, void *ptr)
{
	TVPImportFuncs.Add(name, ptr);
}
...
{
	if(TVPImportFuncsInit) return;
	TVPImportFuncsInit = true;
...
//---------------------------------------------------------------------------
struct tTVPFunctionImporter : iTVPFunctionExporter
{
	...
	bool QueryFuncion(ttstr name, void **function);
} static TVPFunctionImporter;
//---------------------------------------------------------------------------
bool tTVPFunctionImporter::QueryFuncion(ttstr name, void **function)
{
	bool ret = true;
	void** ptr = TVPImportFuncs.Find(name);
	if(ptr)
		*function = *ptr;
	else
		*function = NULL, ret = false;
	return ret;
}

bool TJS_INTF_METHOD tTVPFunctionImporter::QueryFunctions(const tjs_char **name, void **function,
		tjs_uint count)
{
	...
	{
		tname = name[i];
		void ** ptr = TVPImportFuncs.Find(tname);
		if(ptr)
			function[i] = *ptr;
		else
...
//---------------------------------------------------------------------------
bool TJS_INTF_METHOD tTVPFunctionImporter::QueryFunctionsByNarrowString(
...
// #endif
	...
	tTVPStaticPlugin(const ttstr & name);
	~tTVPStaticPlugin();
...
// 	}
	...
	try
	{
		...
		TVPFunctionImporter.QueryFuncion(plugin_name+TJS_W("_V2Unlink"),&fun);
		V2Unlink = (tTVPV2UnlinkProc) fun;
...
//---------------------------------------------------------------------------
tTVPStaticPlugin::~tTVPStaticPlugin()
{
}
//---------------------------------------------------------------------------
bool tTVPStaticPlugin::Uninit()
{
...
typedef std::vector<tTVPStaticPlugin*> tTVPPluginVectorType;
