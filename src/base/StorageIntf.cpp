...
tTVPStorageMediaManager::tTVPStorageMediaManager()
{
	...
	// iTVPStorageMedia *filemedia = NULL;//TODO
	...
	ttstr *ret_media, ttstr *ret_domain, ttstr *ret_path)
{
	...
	ttstr tmp(name);
	TVPPreNormalizeStorageName(tmp);//TODO
	...
	//unix like /a/b/c.txt -- > file://.//a/b/c.txt
	...
	if(media.IsEmpty())
	{
		assert(!TVPCurrentMedia.IsEmpty());
	...
	mediarec->MediaIntf.GetObjectNoAddRef()->NormalizeDomainName(domain); //make char small
	...
	mediarec->MediaIntf.GetObjectNoAddRef()->NormalizePathName(path); // make char small.
	...
	tmp = media + TJS_W("://") + domain + TJS_W("/") + path;
	//wprintf(TJS_W("normalize:%ls-->%ls\n"),name.c_str(),tmp.c_str());
...
ttstr filename_to_lowercase(const ttstr & name)
{
    ttstr _name = name;
    tjs_char* p = _name.Independ();
    tjs_char* pe = p + _name.length();
    tjs_char* pe2 = pe - 1;
    while(pe2!=p && *pe2 != TJS_W('/'))
    {
        --pe2;
    }
    while(pe2!=pe)
    {
        if(*pe2>=TJS_W('A') && *pe2<=TJS_W('Z'))
            *pe2 = *pe2 + (TJS_W('a')-TJS_W('A'));
        ++pe2;
    }
    return _name;
}
...
// TVPGetPlacedPath
//---------------------------------------------------------------------------
ttstr TVPGetPlacedPath(const ttstr & _name)
{
	...
	// found.
    ttstr name = filename_to_lowercase(_name);
