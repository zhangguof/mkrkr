...
static void PostRegistCallback()
{
	...
	auto tmp = tTJSVariant(OPTION_INDENT);
	if (global) {
		global->PropSet(TJS_MEMBERENSURE|TJS_IGNOREPROP, TJS_W("ssoIndent"), NULL, &tmp,  global);
		tmp = tTJSVariant(OPTION_CONST);
		global->PropSet(TJS_MEMBERENSURE|TJS_IGNOREPROP, TJS_W("ssoConst"),  NULL, &tmp,   global);
		tmp = tTJSVariant(OPTION_KEYSORT);
		global->PropSet(TJS_MEMBERENSURE|TJS_IGNOREPROP, TJS_W("ssoSort"),   NULL, &tmp, global);
		tmp = tTJSVariant(OPTION_HIDDEN);
		global->PropSet(TJS_MEMBERENSURE|TJS_IGNOREPROP, TJS_W("ssoHidden"), NULL, &tmp, global);
		global->Release();
	}
}
