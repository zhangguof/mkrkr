...
static void log(const tjs_char *format, ...)
{
	...
	vswprintf(msg,1024,format,args);
	TVPAddLog(msg);
	va_end(args);
}
...
public:
	tjs_error TJS_INTF_METHOD FuncCall(
		...
		auto ir = IStringReader(param[0]->GetString());
		eval(ir, result);
		return TJS_S_OK;
	}
};
...
public:
	tjs_error TJS_INTF_METHOD FuncCall(
		...
		auto ir = IFileReader(filename,codepage);
		eval(ir, result);
		return TJS_S_OK;
	}
};
...
extern "C" HRESULT json_V2Link(iTVPFunctionExporter *exporter)
{
	// スタブの初期化(必ず記述する)
...
extern "C" HRESULT json_V2Unlink()
{
