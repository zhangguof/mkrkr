...
// void _tcscpy_s(const wchar_t* src, int size, const)
...
extern "C" DLL_EXPORT HRESULT STDCALL menu_V2Link(iTVPFunctionExporter *exporter)
{
	printf("%s\n", "load Plugins menu!!");
...
//---------------------------------------------------------------------------
extern "C" DLL_EXPORT HRESULT STDCALL menu_V2Unlink()
{
	...
	// 大きくなっていれば失敗ということにする。
	if(TVPPluginGlobalRefCount > GlobalRefCountAtInit) return -1;
