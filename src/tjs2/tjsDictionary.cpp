...
void tTJSDictionaryNI::Assign(iTJSDispatch2 * dsp, bool clear)
{
	...
	else
	{
		...
		auto tmp1 = tTJSVariantClosure(&countCallback, NULL);
		dsp->EnumMembers(TJS_IGNOREPROP, &tmp1, dsp);
		...
		auto tmp = tTJSVariantClosure(&callback, NULL);

		dsp->EnumMembers(TJS_IGNOREPROP, &tmp, dsp);
	...
	iTJSTextWriteStream & stream, const ttstr &indentstr)
{
	...
	auto tmp = tTJSVariantClosure(&callback, NULL);
	Owner->EnumMembers(TJS_IGNOREPROP, &tmp, Owner);
...
{
	tSaveMemberCountCallback countCallback;
	auto tmp = tTJSVariantClosure(&countCallback, NULL);
	Owner->EnumMembers(TJS_IGNOREPROP, &tmp, Owner);
	...
	callback.Stack = &stack;
	callback.Stream = &stream;
	auto tmp2 = tTJSVariantClosure(&callback, NULL);
	Owner->EnumMembers(TJS_IGNOREPROP, &tmp2, Owner);
}
//---------------------------------------------------------------------------
	...
	std::vector<iTJSDispatch2 *> &stack)
{
		...
		ClassID_Dictionary, (iTJSNativeInstance**)&dicni)) )
	{
		...
		try
		{
			...
			callback.Stack = &stack;
			auto tmp2 = tTJSVariantClosure(&callback, NULL);
			dsp->EnumMembers(TJS_IGNOREPROP, &tmp2, dsp);
		}
		catch(...)
		{
			stack.pop_back();
