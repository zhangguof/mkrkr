...
size_t TJS_mbstowcs(tjs_char *pwcs, const tjs_nchar *s, size_t n)
{
	...
	if(pwcs)
	{
		...
		size_t count = TVPUtf8ToWideCharString(s, pwcs);
		if( n > count )
		{
			pwcs[count] = TJS_W('\0');
		}
	...
	{	// count length
		...
		return TVPUtf8ToWideCharString(s, NULL);
	}
}
//---------------------------------------------------------------------------
//wchar_t --> utf-8
size_t TJS_wcstombs(tjs_nchar *s, const tjs_char *pwcs, size_t n)
{
	...
	if(s)
	{
		...
		tjs_size count = TVPWideCharToUtf8String(pwcs,s);
		if( n > count )
		{
			s[count] = '\0';
		}
	...
	else
	{
		...
		return TVPWideCharToUtf8String(pwcs,NULL);
	}
}
//---------------------------------------------------------------------------
// 使われていないようなので未確認注意
