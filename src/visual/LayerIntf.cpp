	...
	tTVPLayerType type, tjs_int opacity)
{
	...
	if(bmp != UpdateBitmapForChild)
	{
		if(MainImage == NULL)
		{
			...
			nr.Or(destrect);
			nr.Sub(DrawnRegion);
			tTVPComplexRect or1; // operation region
			...
			// operate r
			tTVPComplexRect::tIterator it = or1.GetIterator();
			while(it.Step())
			{
				tTVPRect r(*it);
				tTVPRect sr;
	...
	while(it.Step())
	{
		...
		if(TVPGraphicSplitOperationType != gsotNone)
		{
			...
			if(TVPGraphicSplitOperationType == gsotInterlace)
			{
				...
				{
					or1.top = y;
					or1.bottom = (y+oh < r.bottom) ? y+oh: r.bottom;

					// call "Draw" to draw to the window
					Draw(drawable, or1, false);
				}
			...
			else if(TVPGraphicSplitOperationType == gsotBiDirection)
			{
				...
				else
				{
					...
					while(1)
					{
						or1.top = (y < r.top ? r.top : y);
						or1.bottom = (y+oh < r.bottom) ? y+oh: r.bottom;

						if(or1.bottom <= r.top) break;
...
TJS_BEGIN_NATIVE_PROP_DECL(mainImageBuffer)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Layer);;
		*result = reinterpret_cast<const void*>(_this->GetMainImagePixelBuffer());
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(mainImageBufferForWrite)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Layer);;
		*result = reinterpret_cast<const void*>(_this->GetMainImagePixelBufferForWrite());
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(provinceImageBuffer)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Layer);;
		*result = reinterpret_cast<const void*>(_this->GetProvinceImagePixelBuffer());
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
...
TJS_BEGIN_NATIVE_PROP_DECL(provinceImageBufferForWrite)
{
	...
	{
		TJS_GET_NATIVE_INSTANCE(/*var. name*/_this, /*var. type*/tTJSNI_Layer);;
		*result = reinterpret_cast<const void*>(_this->GetProvinceImagePixelBufferForWrite());
		return TJS_S_OK;
	}
	TJS_END_NATIVE_PROP_GETTER
