		...
		iTJSDispatch2 *tjs_obj)
{
	...
	if( numparams >= 1 && param[0]->Type() == tvtObject ) {
		...
		TVPThrowExceptionMessage(TJS_W("Don't support multiple win!"));
		// wprintf(TJS_W("error:do not support mul win!\n"));
		//Form = new SDLWindow(default_win_width,
					// default_win_height,Application, this);
	} else {
		Form = new SDLWindow(default_win_width,default_win_height,
							Application, this);
	}
	return TJS_S_OK;
}
